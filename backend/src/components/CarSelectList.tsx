...
interface CarSelectListProps {
  ...
  value?: bookcarsTypes.CarSupplier;
  supplier: string;
  pickupLocation: string;
  onChange?: (values: bookcarsTypes.CarSupplier[]) => void;
}

const CarSelectList = ({ label, required, multiple, variant, value, supplier, pickupLocation, onChange }: CarSelectListProps) => {
  ...
  const [cars, setCars] = useState<bookcarsTypes.CarSupplier[]>([]);
  const [openDialog, setOpenDialog] = useState(false);
  const [closeDialog, setCloseDialog] = useState(false);
  const [reload, setReload] = useState(false);
  const [selectedOptions, setSelectedOptions] = useState<bookcarsTypes.CarSupplier[]>([]);
  ...
  const handleChange = (values: bookcarsTypes.CarSupplier[]) => {
    if (onChange) {
      onChange(values);
    }
  };
