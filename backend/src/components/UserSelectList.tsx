...
const UserSelectList = ({
  ...
  const [selectedOptions, setSelectedOptions] = useState<
    bookcarsTypes.Option[]
  >([]);

  useEffect(() => {
    const _value = multiple
      ? (value as bookcarsTypes.Option[])
      : [value as bookcarsTypes.Option];
  ...
  const fetchData = async (
    _page: number,
    _keyword: string,
    onFetch?: bookcarsTypes.DataEvent<bookcarsTypes.User>,
  ) => {
    try {
      setLoading(true);
      ...
      const _drivers =
        _page === 1 ? driverOptionsList : [...drivers, ...driverOptionsList];
