...
const MileageFilter = ({ className, onChange }: MileageFilterProps) => {
  ...
  const handleLimitedMileageChange = (
  ...
  const handleUnlimitedMileageChange = (
    e: React.ChangeEvent<HTMLInputElement> | React.MouseEvent<HTMLElement>,
  ) => {
    if ("checked" in e.currentTarget && e.currentTarget.checked) {
  ...
  return (
    <Accordion
      ...
      className={`${className ? `${className} ` : ""}mileage-filter`}
    >
      <div className="filter-elements">
        <div className="filter-element">
          ...
          />
          <span role="button" tabIndex={0} onClick={handleLimitedMileageClick}>
            {strings.LIMITED}
          </span>
        </div>
        <div className="filter-element">
