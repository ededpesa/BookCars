...
const UserList = ({
  ...
  const getColumns = (
    _user: bookcarsTypes.User,
  ): GridColDef<bookcarsTypes.User>[] => {
    const _columns: GridColDef<bookcarsTypes.User>[] = [
      {
        ...
        renderCell: ({
          row,
          value,
        }: GridRenderCellParams<bookcarsTypes.User, string>) => {
          const __user = row;
          let userAvatar;
      ...
      },
      {
        ...
        >) => (
          <span className={`bs us-${value?.toLowerCase()}`}>
            {helper.getUserType(value)}
          </span>
        ),
        valueGetter: (value: string) => value,
      },
      {
        ...
        renderCell: ({ row }: GridRenderCellParams<bookcarsTypes.User>) => {
          ...
          const __user = row;
          return _user.type === bookcarsTypes.RecordType.Admin ||
            __user.supplier === _user._id ? (
            <div>
    ...
    try {
      ...
      if (status === 200) {
        if (selectedIds.length > 0) {
          setRows(
            rows.filter((row) => !selectedIds.includes(row._id as string)),
          );
        } else {
