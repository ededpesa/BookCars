...
beforeAll(async () => {
  ...
  SUPPLIER1_ID = await testHelper.createSupplier(
    `${SUPPLIER1_NAME}@test.bookcars.ma`,
    SUPPLIER1_NAME,
  );
...
describe("POST /api/validate-supplier", () => {
  it("should validate a supplier", async () => {
    ...
    let payload: bookcarsTypes.ValidateSupplierPayload = {
...
describe("GET /api/all-suppliers", () => {
  it("should get all suppliers", async () => {
    let res = await request(app).get("/api/all-suppliers");
    expect(res.statusCode).toBe(200);
    expect(res.body.length).toBeGreaterThan(1);

    await databaseHelper.close();
    res = await request(app).get("/api/all-suppliers");
    expect(res.statusCode).toBe(400);
...
describe("DELETE /api/delete-supplier/:id", () => {
  it("should delete a supplier", async () => {
    ...
    let supplierId = await testHelper.createSupplier(
    ...
    await supplier?.save();
    let locationId = await testHelper.createLocation(
      "Location 1 EN",
      "Location 1 FR",
    );
    ...
    supplierId = await testHelper.createSupplier(
      `${supplierName}@test.bookcars.ma`,
      supplierName,
    );
    supplier = await User.findById(supplierId);
    ...
    await supplier?.save();
    locationId = await testHelper.createLocation(
      "Location 1 EN",
      "Location 1 FR",
    );
    car = new Car({
