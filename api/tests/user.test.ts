...
describe("GET /api/check-token/:type/:userId/:email/:token", () => {
  it("should check user's token", async () => {
    ...
    let res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Frontend}/${USER1_ID}/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(200);

    res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Backend}/${USER1_ID}/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Frontend}/${USER2_ID}/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Frontend}/${testHelper.GetRandromObjectIdAsString()}/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Frontend}/${USER1_ID}/${USER1_EMAIL}/${uuid()}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).get(
      `/api/check-token/${bookcarsTypes.AppType.Frontend}/0/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(400);
  });
});
...
describe("GET /api/confirm-email/:email/:token", () => {
  it("should send confirmation email", async () => {
    ...
    expect(token?.length).toBeGreaterThan(1);
    let res = await request(app).get(
      `/api/confirm-email/${USER1_EMAIL}/${token}`,
    );
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).get(
      `/api/confirm-email/${testHelper.GetRandomEmail()}/${token}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).get(`/api/confirm-email/${USER1_EMAIL}/${uuid()}`);
    expect(res.statusCode).toBe(400);

    res = await request(app).get(`/api/confirm-email/unknown/${uuid()}`);
    expect(res.statusCode).toBe(400);
  });
});
...
describe("POST /api/resend/:type/:email/:reset", () => {
  it("should resend validation email", async () => {
    ...
    let res = await request(app).post(
      `/api/resend/${bookcarsTypes.AppType.Frontend}/${USER1_EMAIL}/${reset}`,
    );
    expect(res.statusCode).toBe(200);
    user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    ...
    res = await request(app).post(
      `/api/resend/${bookcarsTypes.AppType.Backend}/${ADMIN_EMAIL}/${reset}`,
    );
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post(
      `/api/resend/${bookcarsTypes.AppType.Backend}/${USER1_EMAIL}/${reset}`,
    );
    expect(res.statusCode).toBe(403);

    res = await request(app).post(
      `/api/resend/${bookcarsTypes.AppType.Frontend}/${testHelper.GetRandomEmail()}/${reset}`,
    );
    expect(res.statusCode).toBe(204);

    res = await request(app).post(
      `/api/resend/${bookcarsTypes.AppType.Frontend}/unknown/${reset}`,
    );
    expect(res.statusCode).toBe(400);
  });
});
...
describe("POST /api/update-avatar/:userId", () => {
  it("should update user's avatar", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const filename = res.body as string;
    let avatarExists = await helper.exists(
      path.resolve(env.CDN_USERS, filename),
    );
    expect(avatarExists).toBeTruthy();
    const user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
...
describe("GET /api/check-password/:id/:password", () => {
  it("should check password", async () => {
    ...
    let res = await request(app)
      .get(
        `/api/check-password/${USER1_ID}/${encodeURIComponent(USER1_PASSWORD)}`,
      )
      .set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app)
      .get(
        `/api/check-password/${testHelper.GetRandromObjectIdAsString()}/some-password`,
      )
      .set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
...
describe("POST /api/delete-users", () => {
  it("should delete users", async () => {
    ...
    const supplierId = await testHelper.createSupplier(
      `${supplierName}@test.bookcars.ma`,
      supplierName,
    );
    const locationId = await testHelper.createLocation(
      "Location 1 EN",
      "Location 1 FR",
    );
