...
export const getToken = (cookie: string) => {
  ...
  const token = cookieParser.signedCookie(
    (signedCookie.match(`${env.X_ACCESS_TOKEN}=(s:.*?);`) ?? [])[1],
    env.COOKIE_SECRET,
  ) as string;
  return token;
};
...
export const signinAsAdmin = () =>
  signin(bookcarsTypes.AppType.Backend, ADMIN_EMAIL);

export const signinAsUser = () =>
  signin(bookcarsTypes.AppType.Frontend, USER_EMAIL);
...
export const GetRandomEmail = () =>
  `random.${uuid()}.${Date.now()}@test.bookcars.ma`;
