...
describe("GET /api/location/:id/:language", () => {
  it("should get a location", async () => {
    ...
    let res = await request(app).get(
      `/api/location/${LOCATION_ID}/${language}`,
    );
    expect(res.statusCode).toBe(200);
    expect(res.body?.name).toBe(
      LOCATION_NAMES.filter((v) => v.language === language)[0].name,
    );

    res = await request(app).get(
      `/api/location/${testHelper.GetRandromObjectIdAsString()}/${language}`,
    );
    expect(res.statusCode).toBe(204);
...
describe("GET /api/locations/:page/:size/:language", () => {
  it("should get locations", async () => {
    ...
    let res = await request(app).get(
      `/api/locations/${testHelper.PAGE}/${testHelper.SIZE}/${language}?s=${LOCATION_NAMES[0].name}`,
    );
    expect(res.statusCode).toBe(200);
    expect(res.body.length).toBe(1);

    res = await request(app).get(
      `/api/locations/unknown/${testHelper.SIZE}/${language}`,
    );
    expect(res.statusCode).toBe(400);
  });
});
