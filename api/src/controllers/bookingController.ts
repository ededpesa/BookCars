...
export const confirm = async (
  ...
  const { language } = user;
  const locale =
    language === "fr" ? "fr-FR" : language === "es" ? "es-VE" : "en-US";
  ...
  const mailOptions: nodemailer.SendMailOptions = {
    ...
    html:
      `<p>
        ...
        ${i18n.t("BOOKING_CONFIRMED_PART3")}${car.supplier.fullName}${i18n.t("BOOKING_CONFIRMED_PART4")}${pickupLocationName}${i18n.t(
  ...
  try {
    ...
    if (driver) {
      ...
      const mailOptions: nodemailer.SendMailOptions = {
        ...
        ${helper.joinURL(env.FRONTEND_HOST, "activate")}/?u=${encodeURIComponent(user.id)}&e=${encodeURIComponent(user.email)}&t=${encodeURIComponent(
          token.token,
        )}<br><br>
        ${i18n.t("REGARDS")}<br>
        </p>`,
...
export const updateStatus = async (req: Request, res: Response) => {
  try {
    ...
    logger.error(
      `[booking.updateStatus] ${i18n.t("DB_ERROR")} ${JSON.stringify(req.body)}`,
      err,
    );
    return res.status(400).send(i18n.t("DB_ERROR") + err);
  }
...
export const deleteBookings = async (req: Request, res: Response) => {
  try {
    ...
    logger.error(
      `[booking.deleteBookings] ${i18n.t("DB_ERROR")} ${JSON.stringify(req.body)}`,
      err,
    );
    return res.status(400).send(i18n.t("DB_ERROR") + err);
  }
